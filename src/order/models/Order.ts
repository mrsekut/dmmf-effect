...
export const PricedOrder = Schema.Struct({
  ...
  shippingAddress: Address,
  billingAddress: Address,
  orderLines: Schema.NonEmptyArray(PricedOrderLine),
...
export const changeOrderLinePrice = (
  ...
  const updateLine = (line: PricedOrderLine) =>
...
if (import.meta.vitest) {
  ...
  const createOrderLine = (
    ...
    const productCode_ = Schema.decodeSync(ProductCodeFromString)(productCode);
    const quantity_ = mkOrderQuantity(productCode_, quantity);
  ...
  const createValidatedOrder = (
    orderLines: Array.NonEmptyArray<PricedOrderLine>,
    amountToBill = 0,
  ) =>
    PricedOrder.make({
      type: 'PricedOrder',
